...
/**
 * La photo d'un candidat, pour les rangs de la liste et l'en-tête de la fiche.
 *
 * Même dessin que pour les référents, et le même ordre — celui que le projet
 * impose, jamais un autre :
 *
 *   1. la session — `supabaseServer()` porte celle de la personne connectée ;
 *   2. le droit métier — `chemin_photo_candidat()` ne rend le chemin que si
 *      cette personne a le droit de voir ce candidat dans sa liste
 *      (migration 0019). C'est la base qui tranche, pas cette route ;
 *   3. alors seulement `supabaseAdmin()`, pour lire les octets d'un seau qui
 *      reste fermé.
 *
...
export async function GET(_requete: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: chemin } = await supabase.rpc('chemin_photo_candidat', { candidat: id });
  ...
  return new NextResponse(await data.arrayBuffer(), {
    headers: {
      ...
      // Privé, et court : le droit de voir ce visage peut être retiré, et un
